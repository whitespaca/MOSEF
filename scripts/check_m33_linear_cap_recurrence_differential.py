"""Independently validate the M33 recurrence and repair certificates."""
...
def main() -> int:
    """Check frozen M33 records with compact, dense, Rust, and C# paths."""
    data = json.loads(
        (ROOT / "schemas/m33-linear-cap-recurrence-v1.json").read_text(
            encoding="utf-8"
        )
    )
    env = os.environ.copy()
    ...
    for vector in data["primitive_exit_vectors"]:
        ...
        if expected_mask != vector["expected_mask"]:
            raise AssertionError("registered M33 primitive mask changed")
        ...
        for executable, env_arg in (([str(rust)], None), (csharp, env)):
            ...
            if actual_mask != expected_mask:
                raise AssertionError("M33 primitive exit disagreement")
    ...
    if (
        ...
        raise AssertionError("registered M33 repair profile changed")
    ...
    if list(dense_signatures) != certificate["restricted_signatures"]:
        raise AssertionError("dense M33 construction certificate changed")
    if len(set(dense_signatures)) != len(dense_signatures):
        raise AssertionError("dense M33 construction certificate collides")
    dense_pair_checks = (
        len(dense_signatures) * (len(dense_signatures) - 1) // 2
    )

    failed = data["failed_profile"]
    collision_bucket = failed["collision_buckets"][0]
    ...
    for descriptor in diversified_exceptional_selector(
        ...
        if len(set(masks)) != 1:
            raise AssertionError(
                "registered M33 recurrence collision no longer collides: "
                f"{descriptor.key}"
            )
        dense_collision_descriptor_checks += 1

    print(
        "M33 linear-cap recurrence differential validation: PASS "
