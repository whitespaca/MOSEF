"""Independently validate the M34 recurrence and repair certificates."""
...
def main() -> int:
    """Check frozen M34 records with compact, dense, Rust, and C# paths."""
    data = json.loads(
        (ROOT / "schemas/m34-next-envelope-v1.json").read_text(
            encoding="utf-8"
        )
    )
    env = os.environ.copy()
    ...
    for vector in data["primitive_exit_vectors"]:
        ...
        if expected_mask != vector["expected_mask"]:
            raise AssertionError("registered M34 primitive mask changed")
        ...
        for executable, env_arg in (([str(rust)], None), (csharp, env)):
            ...
            if actual_mask != expected_mask:
                raise AssertionError("M34 primitive exit disagreement")
    ...
    if (
        ...
        raise AssertionError("registered M34 repair profile changed")
    ...
    if list(dense_signatures) != certificate["restricted_signatures"]:
        raise AssertionError("dense M34 construction certificate changed")
    if len(set(dense_signatures)) != len(dense_signatures):
        raise AssertionError("dense M34 construction certificate collides")
    dense_pair_checks = (
        len(dense_signatures) * (len(dense_signatures) - 1) // 2
    )

    failed = data["failed_profile"]
    dense_failed_descriptor_bucket_checks = 0
    for descriptor in diversified_exceptional_selector(
        data["input_length"],
        failed["selector_cap"],
    ):
        for collision_bucket in failed["collision_buckets"]:
            masks = tuple(
                dense_exit_mask(descriptor, prime)
                for prime in collision_bucket
            )
            if len(set(masks)) != 1:
                raise AssertionError(
                    "registered M34 failed-schedule bucket separated: "
                    f"{descriptor.key}"
                )
            dense_failed_descriptor_bucket_checks += 1
    ...
    for descriptor in diversified_exceptional_selector(
        ...
        if len(set(masks)) != 1:
            raise AssertionError(
                "registered M34 predecessor collision separated: "
                f"{descriptor.key}"
            )
        dense_predecessor_descriptor_checks += 1

    print(
        "M34 next-envelope differential validation: PASS "
        f"({cross_language_checks} cross-language command checks, "
        f"{dense_pair_checks} dense certificate pair checks, "
        f"{dense_failed_descriptor_bucket_checks} dense failed-schedule "
        "descriptor-bucket checks, "
