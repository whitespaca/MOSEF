"""Independently validate the M36 distinct-cap certificates."""
...
def dense_collision_descriptor_checks(
    ...
    if not isinstance(collision_buckets, list) or len(collision_buckets) != 1:
        raise AssertionError("M36 profile must contain one collision bucket")
    bucket = collision_buckets[0]
    if not isinstance(bucket, list):
        raise AssertionError("M36 collision bucket has invalid shape")
    ...
    for descriptor in diversified_exceptional_selector(
        ...
        if len(set(masks)) != 1:
            raise AssertionError(
                "registered M36 collision separated: "
...
def main() -> int:
    """Check frozen M36 records with compact, dense, Rust, and C# paths."""
    data = json.loads(
        (ROOT / "schemas/m36-distinct-cap-v1.json").read_text(
            encoding="utf-8"
        )
    )
    env = os.environ.copy()
    ...
    for vector in data["primitive_exit_vectors"]:
        ...
        if expected_mask != vector["expected_mask"]:
            raise AssertionError("registered M36 primitive mask changed")
        ...
        for executable, env_arg in (([str(rust)], None), (csharp, env)):
            ...
            if actual_mask != expected_mask:
                raise AssertionError("M36 primitive exit disagreement")
    ...
    if (
        ...
        raise AssertionError("registered M36 repair profile changed")
    ...
    if list(dense_signatures) != certificate["restricted_signatures"]:
        raise AssertionError("dense M36 construction certificate changed")
    if len(set(dense_signatures)) != len(dense_signatures):
        raise AssertionError("dense M36 construction certificate collides")
    dense_pair_checks = (
    ...
    print(
        "M36 distinct-cap differential validation: PASS "
