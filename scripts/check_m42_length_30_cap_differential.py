"""Independently validate the M42 length-30 cap certificates."""
...
def collision_descriptor_checks(
    ...
    if not isinstance(collision_buckets, list):
        raise AssertionError("M42 collision buckets have the wrong shape")
    checks = 0
    for descriptor in diversified_exceptional_selector(
        ...
        for bucket in collision_buckets:
            if not isinstance(bucket, list) or len(bucket) < 2:
                raise AssertionError("M42 collision bucket is malformed")
            masks = tuple(
            ...
            if len(set(masks)) != 1:
                raise AssertionError(
                    "registered M42 collision separated: "
...
def _transition_checks(
    ...
    """Reconstruct every post-cap-112 bucket with repeated multiplication."""
    first = records[0]
    buckets = first["collision_buckets"]
    if not isinstance(buckets, list) or len(buckets) != 1:
        raise AssertionError("M42 transition root bucket changed")
    ...
    for record in records[1:]:
        ...
        if not previous_keys <= current_keys:
            raise AssertionError("independent M42 inclusion failed")
        added = tuple(
            descriptor
        ...
        if len(added) != int(record["new_descriptor_count"]):
            raise AssertionError("independent M42 transition count changed")
        ...
        if observed_buckets != record["collision_buckets"]:
            raise AssertionError("independent M42 transition bucket changed")
        if observed_pairs != int(record["collision_pair_count"]):
            raise AssertionError("independent M42 transition pairs changed")
...
def main() -> int:
    """Check frozen M42 records with naive, dense, Rust, and C# paths."""
    data = json.loads(
        (ROOT / "schemas/m42-length-30-cap-v1.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    if recomputed_hash != summary_hash:
        raise AssertionError("M42 canonical summary hash changed")
    ...
    for vector in data["primitive_exit_vectors"]:
        ...
        if expected_mask != vector["expected_mask"]:
            raise AssertionError("registered M42 primitive mask changed")
        if independent_exit_mask(descriptor, vector["prime"]) != expected_mask:
            raise AssertionError("independent M42 primitive mask changed")
        if (
            protocol_reference.dense_exit_mask(descriptor, vector["prime"])
            != expected_mask
        ):
            raise AssertionError("dense M42 primitive mask changed")
        ...
        for executable, env_arg in (([str(rust)], None), (csharp, env)):
            ...
            if actual_mask != expected_mask:
                raise AssertionError("M42 primitive exit disagreement")
    ...
    if list(independent_signatures) != certificate["restricted_signatures"]:
        raise AssertionError("independent M42 construction changed")
    if len(set(independent_signatures)) != len(independent_signatures):
        raise AssertionError("independent M42 construction collides")
    ...
    if observed_repairs != expected_repairs:
        raise AssertionError(
            f"independent M42 repair coordinates changed: {observed_repairs}"
        )
    if data["exact_length_30_threshold"] != 123:
        raise AssertionError("M42 exact threshold changed")
    if data["repaired_additive_schedule"]["cap"] != "m+93":
        raise AssertionError("M42 additive repair changed")
    if (
        data["repaired_multiplicative_schedule"]["working_witness"]
        != "ceil(49m/12)"
    ):
        raise AssertionError("M42 multiplicative repair changed")

    print(
        "M42 length-30 cap differential validation: PASS "
