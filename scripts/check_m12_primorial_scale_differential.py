"""Differentially check selected M12 factor-scale primorial vectors."""
...
def load_vectors() -> dict[str, list[dict[str, Any]]]:
    """Load the selected M12 cross-language vectors."""
    path = ROOT / "schemas" / "m12-primorial-scale-vectors-v1.json"
    ...
    if any(not isinstance(value.get(key), list) for key in keys):
        raise TypeError("M12 vectors must contain all five vector arrays")
...
def main() -> int:
    ...
    for vector in vectors["scale_bound_vectors"]:
        bound = primorial_factor_scale_bound(
            vector["prime_count"], vector["target_max"]
        )
        registered = (
            bound.support_limit,
    ...
    print(f"M12 primorial-scale differential validation: PASS ({checks} checks)")
    return 0
