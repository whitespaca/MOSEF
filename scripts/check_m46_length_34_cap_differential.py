"""Independently validate the M46 length-34 cap certificates."""
...
FINAL_COLLISION = [97927, 99527]
REPAIR_SOURCE = "phi6:149:201:45:cofactor"


def main() -> int:
    """Check M46 records with formula, dense, Rust, and C# paths."""
    ...
    for vector in data["primitive_exit_vectors"]:
        ...
        if expected_mask != vector["expected_mask"]:
            raise AssertionError("registered M46 primitive mask changed")
        if independent_exit_mask(descriptor, vector["prime"]) != expected_mask:
            raise AssertionError("independent M46 primitive mask changed")
        if (
            protocol_reference.dense_exit_mask(descriptor, vector["prime"])
            != expected_mask
        ):
            raise AssertionError("dense M46 primitive mask changed")
        ...
        for executable, env_arg in (([str(rust)], None), (csharp, env)):
            ...
            if actual_mask != expected_mask:
                raise AssertionError("M46 primitive exit disagreement")
    ...
    if list(independent_signatures) != certificate["restricted_signatures"]:
        raise AssertionError("independent M46 construction changed")
    if len(set(independent_signatures)) != len(independent_signatures):
        raise AssertionError("independent M46 construction collides")
    ...
    predecessor_buckets = [
        bucket for bucket in predecessor_groups.values() if len(bucket) > 1
    ]
    if predecessor_buckets != [FINAL_COLLISION]:
        raise AssertionError("independent M46 predecessor changed")
    ...
    if observed_repairs != {(1, 0): [REPAIR_SOURCE]}:
        raise AssertionError(
            f"independent M46 repair coordinates changed: {observed_repairs}"
        )
    if data["exact_length_34_threshold"] != 201:
        raise AssertionError("M46 exact threshold changed")
    if data["repaired_additive_schedule"]["cap"] != "m+167":
        raise AssertionError("M46 additive repair changed")
    if (
        data["repaired_multiplicative_schedule"]["working_witness"]
        != "ceil(53m/9)"
    ):
        raise AssertionError("M46 multiplicative repair changed")

    print(
        "M46 length-34 cap differential validation: PASS "
