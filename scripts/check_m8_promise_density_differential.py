"""Differentially check selected M8 signatures, asymmetries, and hit counts."""
...
def load_vectors() -> dict[str, list[dict[str, Any]]]:
    """Load the selected M8 cross-language vectors."""
    path = ROOT / "schemas" / "m8-promise-density-vectors-v1.json"
    with path.open(encoding="utf-8") as handle:
        value = json.load(handle)
    keys = ("signature_vectors", "asymmetry_vectors", "hit_count_vectors")
    if any(not isinstance(value.get(key), list) for key in keys):
        raise TypeError("M8 vectors must contain all three vector arrays")
...
def main() -> int:
    ...
    print(f"M8 promise-density differential validation: PASS ({checks} checks)")
    return 0
