"""Independently validate the M38 length-26 cap certificates."""
...
def dense_collision_descriptor_checks(
    ...
    if not isinstance(collision_buckets, list) or len(collision_buckets) != 1:
        raise AssertionError("M38 profile must contain one collision bucket")
    bucket = collision_buckets[0]
    if not isinstance(bucket, list):
        raise AssertionError("M38 collision bucket has invalid shape")
    ...
    for descriptor in diversified_exceptional_selector(
        ...
        if len(set(masks)) != 1:
            raise AssertionError(
                "registered M38 collision separated: "
...
def main() -> int:
    """Check frozen M38 records with compact, dense, Rust, and C# paths."""
    data = json.loads(
        (ROOT / "schemas/m38-length-26-cap-v1.json").read_text(
            encoding="utf-8"
        )
    )
    env = os.environ.copy()
    ...
    for vector in data["primitive_exit_vectors"]:
        ...
        if expected_mask != vector["expected_mask"]:
            raise AssertionError("registered M38 primitive mask changed")
        ...
        for executable, env_arg in (([str(rust)], None), (csharp, env)):
            ...
            if actual_mask != expected_mask:
                raise AssertionError("M38 primitive exit disagreement")
            cross_language_checks += 2

    multiplicative_record = data["multiplicative_failed_profile"]
    multiplicative = diversified_selector_profile(
        data["input_length"],
        multiplicative_record["selector_cap"],
        compute_minimum_certificate=False,
    )
    if (
        len(multiplicative.population_primes)
        != multiplicative_record["population_size"]
        or multiplicative.descriptor_count
        != multiplicative_record["descriptor_count"]
        or len(multiplicative.normalized_columns)
        != multiplicative_record["normalized_coordinate_count"]
        or multiplicative.collision_buckets
        != tuple(
            tuple(bucket)
            for bucket in multiplicative_record["collision_buckets"]
        )
    ):
        raise AssertionError("registered M38 multiplicative profile changed")
    ...
    if list(dense_signatures) != certificate["restricted_signatures"]:
        raise AssertionError("dense M38 construction certificate changed")
    if len(set(dense_signatures)) != len(dense_signatures):
        raise AssertionError("dense M38 construction certificate collides")
    dense_pair_checks = (
    ...
    print(
        "M38 length-26 cap differential validation: PASS "
