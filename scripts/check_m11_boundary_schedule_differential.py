"""Differentially check selected M11 primorial-boundary vectors."""
...
def load_vectors() -> dict[str, list[dict[str, Any]]]:
    """Load the selected M11 cross-language vectors."""
    path = ROOT / "schemas" / "m11-boundary-schedule-vectors-v1.json"
    ...
    if any(not isinstance(value.get(key), list) for key in keys):
        raise TypeError("M11 vectors must contain all four vector arrays")
...
def main() -> int:
    ...
    print(f"M11 boundary-schedule differential validation: PASS ({checks} checks)")
    return 0
