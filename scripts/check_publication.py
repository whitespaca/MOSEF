...
def main() -> int:
    ...
    for english_path, korean_path in M82_FOCUSED_PAIRS:
        ...
        if english_matches != korean_focused_matches:
            ...
            errors += 1
        if not 5 <= len(english_matches) <= 8:
            fail(
                "M82 focused paper must contain 5--8 representative claims: "
        ...
        for claim_id, status in english_matches:
            ...
            if ledger.get(claim_id) != normalized_status(status):
                ...
                errors += 1
    if len(projected_ids) != 22:
        fail(f"M82 focused projection must contain 22 unique claims, got {len(projected_ids)}")
        errors += 1
