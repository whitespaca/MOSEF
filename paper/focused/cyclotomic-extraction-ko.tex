...
\begin{abstract}
본 논문은 공개된 부호 있는 깊이 2 기하급수 회로
\(F(X)=c_1S_A(X)+c_2S_B(X^A)\)를 연구하고 두 stage와 aggregate 사이의
예외적 공통인수를 만들 수 있는 모든 유리 root-of-unity 비율을
분류한다. 일반적인 common-step 족을 제외하면 이차 예외는 계수 비율
1인 차수 4와 비율 2인 차수 6뿐이다. 각 경우에 정확한 cyclotomic
quotient, 선형 remainder, resultant를 유도한다. 이 cofactor는
\(O(\log A+\log B)\) modular operation을 사용하는 나눗셈 없는
evaluator를 가지며, base, 두 geometric stage, 직접 cyclotomic 값,
공개 overlap GCD가 모두 unit이어도 proper factor를 드러낼 수 있다.
모든 nonunit, inversion, full-collision, extraction branch는 명시적이고
factorization-independent하다. online 비용 모델은 공개 매개변수의
구성과 bit 길이, compact binary 평가, 계수 reduction, 모든 GCD와 실패한
inversion, 요청된 출력을 과금한다. root-of-unity 분류, symbolic
항등식, resultant 유도는 online advice가 아닌 offline 증명 작업이다.
dense exact lift를 materialize하면 실제 출력 크기를 과금하며 그 비용을
compact 평가 안에 숨기지 않는다. 또한 고정된 모든 유한 공개 schedule은
무한히 많은 semiprime을 놓치고, 한 cofactor는 support-signature 한
비트만 제공하며, 켤레 값은 두 번째 채널을 만들지 못하고, 지수적으로
큰 formal lift는 다항 출력 비용을 갖지 않을 수 있음을 증명한다. 따라서
이 결과는 제한된 부호 깊이 2 회로 문법의 branch-total extraction
정리이며 보편 schedule, promise recognizer, 일반 정수분해 알고리즘이
아니다. compact 계산 가능성과 실제 인수분해 성공은 같은 명제가 아니며
두 비용과 조건을 분리한다.
\end{abstract}

\begin{costmodel}{비용 모델 요약}
\costrow{온라인 과금}{공개 매개변수 구성과 encoding, compact stage 및
cofactor 평가, 계수 reduction, 모든 GCD와 실패한 inversion, 인수 추출,
요청된 출력을 포함한다.}
\costrow{오프라인 증명}{root-of-unity 분류, 다항식 항등식, resultant
유도, 반례 탐색, 재현 기록은 online 입력이 아니다.}
\costrow{출력 규칙}{dense 또는 sparse exact materialization은 실제
생성한 표현의 크기를 과금한다. compact modular 평가는 무료 전개를
허용하지 않는다.}
\costrow{보장 범위}{다항 bit 비용은 다항시간에 구성되고 bit 길이가
다항식으로 유계인 공개 매개변수와 명시한 부호 깊이 2 문법에만
적용된다.}
\end{costmodel}
...
\]
를 정의한다. 이후 모든 복잡도 진술은 앞의 비용 표를
\(\bits(N)=\lfloor\log_2N\rfloor+1\)에 적용하며 숨겨진 branch나 출력은
없다.

\readerclaim{명제}{stage 서로소성과 정확한 resultant}
{\claimstatus{BAR-018}{PROVED}}
두 stage는 \(\mathbb Q[X]\)에서 서로
소이고
\[
...
\readerclaim{명제}{total content 및 resultant 축약}
{\claimstatus{BAR-019}{PROVED}}
content \(d\)는 unit, proper, full의
...
\readerclaim{정리}{유리 root-of-unity 비율}
{\claimstatus{THM-003}{PROVED}}
이 비율이 유리수인 경우는 정확히 다음
세 가지이다.
\[
\begin{array}{c|c|c}
...
\((A,B,n)=(2,4,5)\)에서는 \((1+\sqrt5)/2\)가 나온다. 전체 norm
case-split artifact는 부록~\ref{app:cyclotomic-reproduction}에
색인되어 있다.
\end{claimproof}
...
\readerclaim{명제}{나눗셈 없는 예외 cofactor}
{\claimstatus{BAR-020}{PROVED}}
각 \(C_i\)는 차수 \(A(B-1)-2\)인 monic
...
\readerclaim{명제}{branch-total cofactor 추출}
{\claimstatus{BAR-021}{PROVED}}
cofactor GCD는 capped 지역 valuation
...
\readerclaim{장벽}{지수적으로 비싼 exact lift}
...
\readerclaim{장벽}{cofactor support 한 비트}
...
\section{재현성, 한계 및 아카이브 지도}
\label{app:cyclotomic-reproduction}

\paragraph{증명 아카이브.}
전체 norm case split은 다음 경로에 있다.
\par\smallskip
