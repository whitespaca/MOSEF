...
\readerclaim{Theorem}{Hereditary \(p-1\) complete factorization}
{\claimstatus{THM-001}{PROVED}}
For every fixed pair \(B,R\) above, fresh
...
algorithm is always correct.  The fully expanded branch and recursion
artifact is indexed in Appendix~\ref{app:promise-reproduction}.
\end{claimproof}
...
\readerclaim{Counterexample and criterion}{Coverage is not separation}
{\claimstatus{BAR-001}{PROVED}}
Divisor coverage does not imply order
...
\readerclaim{Proposition}{Conjugate channels have identical support}
{\claimstatus{BAR-002}{PROVED}}
If \(a\) is a unit modulo \(N\) and
...
\readerclaim{Proposition}{Exact Lucas root count}
{\claimstatus{LEM-003}{PROVED}}
For every odd prime \(q\) and \(d>0\),
\[
...
\readerclaim{Theorem}{Hereditary nonsplit \(p+1\) complete factorization}
{\claimstatus{THM-002}{PROVED}}
Fresh uniform parameters \(P\bmod K\) give a
...
argument is the one used for THM-001.  The full proof artifact is indexed in
Appendix~\ref{app:promise-reproduction}.
\end{claimproof}
...
reference API are indexed in Appendix~\ref{app:promise-reproduction}.
...
\readerclaim{Barrier}{Common-schedule density ceiling}
{\claimstatus{BAR-003}{PROVED}}
For distinct odd primes \(p,q\), the union
...
\readerclaim{Barrier}{Subcritical exponent-list sparsity}
{\claimstatus{BAR-004}{PROVED}}
An explicit polynomial-size exponent list
...
\label{app:promise-source-positioning}
...
\label{app:promise-reproduction}

\paragraph{Proof and implementation archive.}
The expanded proof and executable anchors are listed below.
\par\smallskip
\noindent\begin{minipage}{\linewidth}
\raggedright\small
\path{research/proofs/THM-001-semismooth-promise.md}\\
