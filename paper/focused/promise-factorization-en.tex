...
\begin{abstract}
We isolate two classical complete-factorization theorems whose hypotheses
are hereditary order asymmetries.  The first uses fresh uniform bases in a
\(p-1\) schedule and succeeds with probability at least \(5/12\) per complete
witness cycle.  The second uses fresh uniform Lucas parameters in a nonsplit
\(p+1\) schedule and succeeds with probability greater than \(1/12\) at a
witness trial. For every finite per-node budget \(s\), bounded versions
terminate on every input with either a complete verified factorization or
\texttt{UNRESOLVED}; on the respective promises their local unresolved tails
are at most \((7/12)^s\) and \((11/12)^s\). No unproved number-theoretic
conjecture is assumed, but
membership in either class is promised and is not recognized by the
algorithms.  We also separate divisor coverage from order-signature
injectivity, prove that the conjugate Lucas map supplies no independent
channel, and record the finite-density limitation of a common schedule.
These are restricted-input results.  They do not give a classical
polynomial-time algorithm for factoring arbitrary integers.
\end{abstract}
...
\]
The input is given in binary, and all construction, exponentiation, GCD,
preprocessing, recursion, and output costs are charged in \(m\).  Exact
primality testing and maximal-perfect-power decomposition are performed
before a splitter is invoked.  A reported divisor is checked before the
two children are recursively processed.
