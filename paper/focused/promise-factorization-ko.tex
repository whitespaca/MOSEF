...
\readerclaim{정리}{유전적 \(p-1\) 완전 인수분해}
{\claimstatus{THM-001}{PROVED}}
위의 고정된 \(B,R\)마다, 새로운 균등
...
artifact는 부록~\ref{app:promise-reproduction}에 색인되어 있다.
\end{claimproof}
...
\readerclaim{반례와 판정}{약수 포함은 차수 분리가 아니다}
{\claimstatus{BAR-001}{PROVED}}
약수 포함은 차수 분리를 함의하지 않는다.
...
\readerclaim{명제}{켤레 채널의 support는 동일하다}
...
\readerclaim{명제}{정확한 Lucas root 개수}
{\claimstatus{LEM-003}{PROVED}}
모든 홀수 소수 \(q\)와 \(d>0\)에 대해
\[
...
\readerclaim{정리}{유전적 비분할 \(p+1\) 완전 인수분해}
{\claimstatus{THM-002}{PROVED}}
새로운 균등 \(P\bmod K\)를 사용하는
...
다항 기대 비용을 준다. 완전한 증명 artifact는
부록~\ref{app:promise-reproduction}에 색인되어 있다.
\end{claimproof}
...
의미론은 부록~\ref{app:promise-reproduction}에 색인되어 있다.
...
\readerclaim{장벽}{공통 schedule의 분포 상한}
{\claimstatus{BAR-003}{PROVED}}
서로 다른 홀수 소수 \(p,q\)에 대해 지역
...
\readerclaim{장벽}{임계 미만 지수 목록의 희소성}
...
\paragraph{증명 및 구현 아카이브.}
확장된 증명과 실행 anchor는 다음과 같다.
\par\smallskip
...
\noindent\begin{minipage}{\linewidth}
\raggedright\small
\path{research/experiments/EXP-0003-m3-semismooth-search.md}\\
\path{research/experiments/EXP-0006-m7-nonsplit-lucas.md}
\end{minipage}
실험은 유한 산술과 branch를 검사하며 확률 하한의 증명을 대체하지 않는다.

전체 증명, 반례, 비용 장부, 부정 결과, claim 의존성은
\archivepaper{}, \path{research/CLAIMS.md},
\path{research/NEGATIVE_RESULTS.md}에 보존된다. 기계 검증되는 투영은
\path{schemas/m82-paper-portfolio-v1.json}이다. 약속 membership은
여전히 인식되지 않고 입력 밀도 정리도 없으며, 일반 고전적 다항시간
인수분해 문제는 열린 상태이다.
