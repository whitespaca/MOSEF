...
\begin{abstract}
본 논문은 고전적 \(p-1\) 및 비분할 \(p+1\) 기법에서 얻은 두 개의
제한적 완전 인수분해 정리를 분리하여 제시한다. 첫 번째 알고리즘은 매
시도마다 새로운 균등 밑을 뽑으며, 약속된 잔여 노드의 완전한 witness
cycle마다 적어도 \(5/12\)의 분할 성공 확률을 갖는다. 두 번째 알고리즘은
독립적인 균등 Lucas 매개변수를 사용하며 witness 시도마다 \(1/12\)보다
큰 성공 확률을 갖는다. 증명되지 않은 수론적 추측은 가정하지 않지만,
두 입력류의 membership은 약속될 뿐 알고리즘이 인식하지 못한다. 또한
약수 포함과 차수 signature 분리를 구별하고, 켤레 Lucas 사상이 독립
채널이 아님을 증명하며, 공통 schedule의 유한 분포 한계를 제시한다.
이는 제한 입력 정리이며 임의의 정수에 대한 고전적 다항시간
인수분해 알고리즘이 아니다.
\end{abstract}
...
\]
로 둔다. 입력은 이진수이며 schedule 구성, 지수승, GCD, 전처리, 재귀,
출력의 모든 비용을 \(m\)에 대해 계산한다. 정확한 소수 판정과 최대
완전거듭제곱 분해를 splitter보다 먼저 수행하고, 보고된 약수는 두
자식으로 재귀하기 전에 검증한다.
...
\end{verbatim}
실험 기록은
\path{research/experiments/EXP-0003-m3-semismooth-search.md}와
\path{research/experiments/EXP-0006-m7-nonsplit-lucas.md}에 있다.
